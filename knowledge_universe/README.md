...
## 內容深度補丁：裸K / Price Action 交易（⚠️ 已撰寫、尚未部署）

使用者實際點開已部署的 `tech_naked_price_action`（第一批卡片）之後回饋內容太薄——
只有一段原理、一個案例、兩條常見錯誤，「皮毛都學不到」。這張卡原本的寫法其實是這批
所有卡片共用的格式（單段原理＋單一案例＋2 條常見錯誤），問題不只這一張卡有，只是
這張先被抓到。

先把這張卡重寫成明顯更深的版本，當作往後補強其他卡片的參考基準：
- **原理**從一段話擴充成四段——「裸K」定義、實體/影線各自代表什麼、幾種常見K線
  組合(錘子線/流星線/吞噬/內包/十字線)各自的意義、以及最容易被誤用的地方(型態
  脫離位置脈絡)該怎麼修正(拿裸K當結構分析的確認層，而不是獨立系統)。
- **案例**從一個情境擴充成三個對照情境——同樣是長下影線，出現在趨勢末端＋放量
  (高品質訊號) vs. 出現在盤整區間中段(雜訊) vs. 疊加在關鍵支撐位置的吞噬型態
  (裸K＋結構疊加的實務用法)，讓「位置決定意義」這個核心提醒有具體對照可以感受，
  不是只用一句話帶過。
- **常見錯誤**從 2 條擴充成 4 條，新增「沒有跟鄰近K線比較相對大小」「低週期圖表
  雜訊過多」這兩個實務上很常踩的坑。
...
**⚠️ 誠實揭露：`tech_naked_price_action` 屬於第一批、已經實際部署在 Supabase 裡**，
所以光改 `seed_knowledge_nodes.sql`(已同步更新，保持「以後重建全新專案」時的一致性)
不會讓現有資料庫裡的內容變新——這批的 INSERT 語法沒有 `on conflict`，直接整批重跑
會因為主鍵重複整批失敗。實際更新現有那一列，需要另外執行
`update_naked_price_action.sql` 這支只改一列的 UPDATE 語法，這個工作階段的
Supabase 寫入工具權限一樣被擋下來，所以還是只能請使用者自己貼到 Supabase
Dashboard 的 SQL Editor 執行。

**還沒處理的部分**：其他已部署卡片(第一批到第五批，加上還沒部署的第六、七批)大概率
也是同樣偏薄的格式，這次只先深度重寫了使用者實際回報的這一張，還沒有系統性地把
全部卡片都拉到這個深度——這是一件量體不小的工作(60 幾張卡片全部重寫)，值得先確認
使用者要不要真的整批做，而不是自作主張全部改掉。
...
├─ update_naked_price_action.sql # 補丁：加深「裸K / Price Action 交易」卡片內容（⚠️ 已撰寫、尚未部署，見下方說明）
