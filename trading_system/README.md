...
本系統圍繞「市場掃描 → 訊號引擎 → 交易規劃 → 風控關卡 → 監控循環 → 最終決策」這六個
階段執行與開發——這不是事後補的說明，是各功能區塊被設計成這樣的原因（主頁上方「系統
架構」那個區塊就是這張表的使用者可見版本）。之後新增功能，先想清楚它屬於哪一階段、
補的是哪一段缺口，不是零散堆疊。
...
**保留但不屬於這六階段的既有功能**：📚 知識宇宙（遊戲化投資知識庫，見下方「知識宇宙分頁」
說明）、🥊 AI 辯論空間、📊 歷史回測、🧮 倉位計算機、📈 K線走勢圖、📲 PWA 可安裝性、
💚 死人開關監控／🖥️ systemd 部署——這些是獨立的輔助/教育/維運功能，跟六階段框架平行
存在，**不會因為聚焦六階段框架而被刪除**。
...
python -m trading_system.pattern_recognition  # 型態辨識 6 種偵測器自測
python -m trading_system.db               # SQLite 儲存層（訊號歷史/警報 CRUD/OI 快照等）自測
python -m trading_system.backtest         # 歷史回測重播邏輯 + 參數掃描排序邏輯自測
```
...
「虧到這裡就認賠出場」的風險控管價位；失效條件是「這個判斷邏輯本身還成不成立」的結構性
價位——做多訊號的判斷邏輯是「站上盒子高點」，如果收盤價又跌回盒子高點以下，代表這次
突破很可能是假突破，訊號的原始論述已經站不住腳，是比停損更早、更輕的示警（做多：
`invalidation_price = box_high`，剛好介於進場價與停損價之間；做空對稱：
...
代表哪個廣為人知的技術分析概念」當方向線索，偵測邏輯是重新設計、可回測驗證的量化定義，
不是照抄圖卡的圖形判讀。**刻意只做 6 種、不是全部 25 種**：

- 杯柄突破／圓弧底起漲／頭肩底突破——需要判斷「形狀夠不夠圓滑/對稱」，簡單規則容易
  產生大量假陽性（隨便一段震盪都能牽強解釋成杯柄或頭肩），比不做還誤導，不實作。
- 漲停突破——台股/中國A股特有的每日漲跌幅限制機制，OKX 永續合約沒有這個概念，不適用。
- 突破籌碼密集區——需要成交量在各價位的分布（volume profile），本系統的 K 線資料只有
  每根的總成交量、沒有價位分布明細，資料精細度不足以可靠判斷，不實作。
...
`outcome_tracker.compute_exposure_gate`：現在同時開著的未結算訊號數（`total_open`，見
下方「持倉組合風險總覽」）達到 `MAX_OPEN_POSITIONS`（預設 5）時，這一輪**所有**新訊號
一律攔截成「⚠️ 曝險已達上限，暫緩新訊號 (EXPOSURE CAP)」——不是這筆新訊號本身技術面/
新聞面不好，是帳戶整體已經同時承擔夠多份風險，不該再疊加。

跟每日虧損斷路器是互補的兩層 portfolio-level 關卡：斷路器看「今天已經發生的虧損」
（事後、時間軸角度），這個看「現在正在承擔的部位數」（當下、部位數量角度）——一個帳戶
...
`outcome_tracker.compute_volatility_gate`：這檔商品的 24h 振幅達到 `MAX_AMPLITUDE_PCT`
（預設 20.0%，含）以上時，訊號攔截成「⚠️ 波動過於劇烈，謹慎評估 (HIGH VOLATILITY)」——
20 EMA + 盒子突破這套策略是為「正常」波動幅度設計、拿歷史資料驗證過參數，波動極端偏離
正常範圍時（例如單日暴漲暴跌），容易出現插針/滑價，驗證過的參數不見得還適用。

跟商品篩選階段的 `MIN_AMPLITUDE_PCT`（見下方「商品篩選門檻」）方向相反：那個濾掉「太
安靜」（振幅太小、雜訊多、不值得分析）的商品，這個濾掉「太劇烈」的商品，兩個門檻搭配
起來才是「波動要落在合理區間」的完整篩選——只設下限、不設上限的話，遇到極端行情一樣
會照常產生訊號，不是使用者想要的。設成 0 代表關閉這道關卡。
...
使用者反應想提高訊號的勝率，問「有沒有辦法」——老實的答案是沒有任何調法能保證某一筆
訊號會贏，但**用同一批真實歷史K線，實測比較很多組參數的表現，取代憑經驗猜一組數字**，
是唯一誠實、可驗證的方向。`backtest.run_sweep()` 把 `box_lookback`／`(tp1_rr, tp2_rr)`／
`volume_confirm_multiple` 的多種組合，各自跑一次 `run_backtest()`（同一套零 look-ahead
bias 的重播邏輯），依「可靠度＋獲利因子」排序，全部回傳給使用者自己比較——不是只丟一個
孤立的「最佳解」，讓使用者也看得到第二名、第三名參數跟冠軍差多少。

任一商品的詳情頁「🔬 掃描最佳參數」按鈕（`POST /api/v1/backtest/sweep`）觸發，一樣
**零 AI 成本**，K 線只多抓一次（限制/防重複打點邏輯跟一般回測一致），掃描本身是記憶體
裡的純運算，不會額外呼叫 OKX API。同時回傳 `baseline`——用「現在系統實際在用」的
`config.py` 預設參數跑一次回測，讓使用者可以直接對照「掃描出來比較好的組合」相對於
「現在的預設值」差多少，不是自己憑感覺去猜落差有多大。

**三個必須讓使用者看到的誠實限制**（前端結果區塊會直接顯示對應文字，不是只寫在文件裡）：
1. **這是「過去這批資料上表現最好」，不是「未來一定最好」**——參數是拿同一批歷史資料
   挑出來的，本質上有過度擬合（overfitting）這批歷史的風險。
2. **樣本數太少（少於 5 筆交易）的組合不可靠**，不管獲利因子/勝率數字多漂亮，用
   `reliable` 欄位明確標示成「樣本太少，僅供參考」，不是偷偷過濾掉也不是假裝每組結果
   都一樣可信。
3. **`ema_period`（20 EMA）刻意不納入掃描範圍**——這是整套策略「20 EMA + 盒子突破」
   品牌識別的核心參數，拿掉它掃描等於在測一個完全不同的策略，不是原本這套規則的參數
   微調；掃描只動盒子回看根數/停利倍數/量能確認門檻這三個更偏「執行細節」的參數。

**防呆**：使用者自訂的掃描範圍如果組合數超過 500 組，端點會明確拒絕並說清楚上限，不會
默默截斷、也不會讓一次請求把伺服器卡住半天。

## 🎯 推薦強度榜只顯示真正可以動作的訊號

同一輪反饋也發現「推薦強度榜」（主畫面的做多/做空排行榜）跟 `brain.fuse()` 最終融合出
來的可執行狀態其實是脫鉤的——榜單原本只檢查 `strategy.py` 有沒有偵測到原始突破
（`signal_type`），沒有檢查這個訊號是不是已經被 `brain.fuse()` 後面的關卡（高週期趨勢
逆向／新聞情緒衝突／手續費吃光淨利／波動過於劇烈）降級成黃色「觀望」——一檔已經被系統
自己判定不該進場的訊號，理論上還是可能因為技術面分數夠高而擠進推薦榜，讓使用者誤以為
是精選的好機會。

修正後，推薦強度榜只收「`brain.fuse()` 最終融合結果是 🔥強烈做多／💥強烈做空（GREEN／RED）」
的訊號——跟「新訊號主動推播通知」用的判斷準則一致（見上方「訊號觸發通知」一節）。
榜單以外的完整卡片列表（主畫面全部訊號卡片）不受影響，仍然照舊完整顯示每一檔商品的
真實狀態，這個過濾只影響「精選推薦」這個特定榜單。
...
**曝險上限（硬性關卡）**：這個面板本身純資訊揭露，不會擋任何東西；`total_open` 達到
`MAX_OPEN_POSITIONS` 才會真的攔截新訊號（見上方「🚧 曝險上限關卡」），面板標題旁會多
顯示「/ 上限 N」，觸發時面板裡也會多一條橘色提示，跟每一張被攔截的訊號卡片上顯示的
「⚠️ 曝險已達上限，暫緩新訊號 (EXPOSURE CAP)」是同一個判斷結果，只是換個地方讓使用者
不用逐張卡片找也能一眼看到。
