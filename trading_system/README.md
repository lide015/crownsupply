...
「全部商品總覽」表格的 價格／24h漲跌／24h振幅／24h成交額／持倉異動 五個欄位標題可以
點擊排序——點一次依該欄位由大到小排序（顯示 ▼），再點一次切換成由小到大（顯示
▲），改點別的欄位會重設回由大到小。預設排序是「24h振幅」由大到小（波動大的商品通常
最值得先看）。某些商品沒有持倉異動資料（例如現貨、或該輪未平倉量快照缺失）時，不管
目前是哪個排序方向，缺值的列一律排在最後面，不會因為排序方向不同而忽大忽小地跳位置。
橫向捲動這張表（欄位很多，小螢幕通常需要捲）時，最左邊的「代號」欄會固定不跟著捲走，
方便對照每一列是哪一檔（`position: sticky; left: 0`，見 `index.html` 的
`_allInstCompare()`／`.all-inst-sort-th`）。表頭同時支援鍵盤操作——`tabindex="0"` 讓
Tab 鍵能聚焦到每個可排序欄位，Enter／Space 觸發跟滑鼠點擊一樣的排序切換，並且
`aria-sort`（`ascending`／`descending`／`none`）會即時反映目前排序狀態，讓螢幕閱讀器
使用者也能知道「目前依哪一欄、哪個方向排序」，不只是視覺上的 ▲▼ 箭頭。

### 📄 分頁載入（顯示更多）

「全部商品總覽」一次可能有 200~300+ 檔，一次把每一列都建成 DOM（含逐列的 ⭐／🔍 分析
按鈕與監聽器）沒有必要，尤其這張表還會被「每 15 秒即時報價輪詢」整個重建一次——預設只
渲染前 50 檔，其餘留在記憶體裡，表格下方「顯示更多（已顯示 X / Y 檔，還有 Z 檔）」按鈕
按一次多展開 50 檔。改篩選分類／商品類型／只看觀察清單／搜尋字串都會重設回第一批 50
檔（篩選條件變了，「已展開到第幾批」這個概念也該重新算）；但單純改排序方式，或
15 秒即時報價輪詢刷新報價，都會保留使用者已經展開的量，不會讓正在看的清單無預警縮
回去。切到「🔥 熱力圖」檢視時「顯示更多」按鈕會跟著隱藏——熱力圖本來就是設計成「一眼
看全市場」用途，不分頁（見 `index.html` 的 `ALL_INST_PAGE_SIZE`／
`allInstVisibleCount`）。

### 💀 骨架載入狀態（Skeleton Loading）

按下「🔍 立即分析」的瞬間，「多空雙向訊號面板」跟「全部商品總覽」表格會立刻換成灰色
閃爍（`animate-pulse`）的骨架佔位區塊，而不是維持空白或舊資料直到 API 回應——讓使用者
清楚知道「正在抓新資料」，而不是誤以為畫面卡住或點擊沒反應。等分析真正完成（不管是
成功、409「已經在分析中」、還是連線失敗）都會換回真實內容，骨架狀態不會卡住。有開啟
系統「減少動態效果」設定的使用者，閃爍動畫會自動關閉、只保留靜態灰色色塊（見
`tailwind.input.css` 的 `prefers-reduced-motion` 覆寫）。
...
可以直接切換開關、調整間隔、看即時狀態，不用另外打 `curl`：

- **開關**：`<input type="checkbox">`，切換時直接呼叫 `POST /api/v1/scheduler`，失敗會
  自動把畫面撥回原狀態（不會顯示一個跟後端實際狀態對不上的假開關）。
- **間隔**：輸入分鐘數 + 「套用」按鈕，送出時換算成秒、伺服器端一樣會強制夾到 900 秒下限。
- **狀態格**：上次執行時間、下次執行倒數、今日 AI 呼叫次數／上限、AI 連續失敗次數／門檻，
  每 10 秒自動輪詢 `GET /api/v1/scheduler` 刷新（純讀 DB／記憶體，零 AI 成本）。
- **跳過原因**：只要這一輪 AI 被節流跳過，面板上會顯示具體原因（例如「省 token 週期：
  規則大腦每輪都跑，AI 新聞情緒每 3 輪才問一次」），不會悶著不說。
- **首次說明橫幅**：排程第一次開啟時會顯示一段說明（會做什麼、不會做什麼），關掉後記在
  `localStorage`，之後不會再跳出來。
