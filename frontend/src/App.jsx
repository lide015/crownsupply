...
/* ─────────────────────────────────────────────
   《節流晨報》— 低 token 金融自動分析站
   架構原則：資料抓取與運算全在本地後端（0 token），
   AI 只在管線最後一哩被呼叫一次，且受條件閘門管制。
   資料來源：本機後端 GET /api/latest、GET /api/klines、WS /ws
   （後端本身抓 CoinGecko／OKX 公開行情，前端不再直連外部 API）
   ───────────────────────────────────────────── */
...
/* ── token 估算（估算值：中英混排約 2.3 字元/token）── */
...
/* ── 小元件 ── */
const Tag = ({ children, color = C.faint }) => (
  <span
